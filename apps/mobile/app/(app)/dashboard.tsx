...
export default function DashboardScreen() {
  ...
  async function submitPeriod(date: Date) {
    // toISOString() converteria para UTC e viraria o dia seguinte após as 21h em UTC-3.
    const iso = [
      date.getFullYear(),
    ...
    if (res.ok) {
      await loadData();
      toast.success("Menstruação registrada.");
    } else {
    ...
    return (
      <SafeAreaView className="flex-1 bg-surface items-center justify-center px-6">
        ...
        <Text className="text-base text-muted text-center mb-6">
          Precisamos de algumas informações para calcular sua fase atual e
          personalizar o app para você.
        </Text>
        <PressableScale
      ...
      >
          ...
          >
            ...
            <View className="flex-row gap-3 mt-1">
              <View
                ...
                style={{ backgroundColor: accent.tint }}
              >
                <Zap size={16} color={accent.color} />
                <View>
                  <Text className="text-sm" style={{ color: accent.color }}>
                    Energia
                  </Text>
              ...
              <View
                ...
                style={{ backgroundColor: accent.tint }}
              >
                <Timer size={16} color={accent.color} />
                <View>
                  <Text className="text-sm" style={{ color: accent.color }}>
                    Dias restantes
                  </Text>
        ...
        {status?.isLate && (
          <Animated.View
            key={`late-${animKey}`}
            entering={FadeInDown.duration(250)}
            className="bg-white rounded-2xl p-5 gap-3 border"
            style={{ borderColor: "#F0C36D" }}
          >
            <View className="flex-row items-center justify-between">
              <View
                className="rounded-2xl p-3"
                style={{ backgroundColor: "#FDF3DF"}}
              >
                <CalendarDays size={26} color="#B4801A" />
              </View>
              <View
                className="px-3 py-1 rounded-full"
                style={{ backgroundColor: "#FDF3DF" }}
              >
                <Text className="text-sm font-medium" style={{ color: "#B4801A" }}>
                  Dia {status.currentDay} do ciclo
                </Text>
              </View>
            </View>
            ...
            <PressableScale
              ...
              haptic
              className="rounded-2xl py-3.5 items-center mt-1"
              style={{ backgroundColor: "#B4801A" }}
            >
              <Text className="text-white font-semibold text-base">
        ...
        >
          <View>
            <Text className="text-sm text-muted">Próxima menstruação</Text>
            <Text className="text-base font-semibold text-foreground">
              {nextPeriod}
            </Text>
          </View>
        ...
        >
          ...
          >
            <View>
              <Text className="text-sm text-muted">Menstruação chegou?</Text>
              <Text className="text-base font-semibold text-foreground">
          ...
          <View className="flex-row items-center justify-between">
            ...
            {saved && (
              <View className="flex-row items-center gap-1">
                <Check size={16} color="#7C6FCD" />
                <Text className="text-base text-primary">Salvo</Text>
              </View>
            )}
          </View>
                    ...
                    {MOOD_OPTIONS.map((opt) => (
                      <PressableScale
                        ...
                        onPress={() => {
                          setLog((prev) => ({
                            ...prev,
                            mood: prev.mood === opt.value ? null : opt.value,
                          }));
                          setSaved(false);
                        }}
                        ...
                        style={{
                          backgroundColor:
                            log.mood === opt.value ? "#7C6FCD" : "#F5F0FF",
                          borderColor:
                            log.mood === opt.value
                              ? "#7C6FCD"
                      ...
                      >
                        <Text
                          className="text-sm font-medium"
                          style={{
                            color: log.mood === opt.value ? "#fff" : "#9ca3af",
                          }}
                        >
                          {opt.label}
                        </Text>
                      ...
                      <PressableScale
                        ...
                        onPress={() => {
                          setLog((prev) => ({
                            ...prev,
                            energy:
                              prev.energy === opt.value ? null : opt.value,
                          }));
                          setSaved(false);
                        }}
                        className="flex-1 py-2 rounded-xl items-center border"
                        style={{
                          backgroundColor:
                            log.energy === opt.value ? "#7C6FCD" : "#F5F0FF",
                          borderColor:
                            log.energy === opt.value
                              ? "#7C6FCD"
                              : "rgba(124,111,205,0.15)",
                        }}
                      >
                        <Text
                          ...
                          style={{
                            color:
                              log.energy === opt.value ? "#fff" : "#9ca3af",
                          }}
                        >
                          {opt.label}
                        </Text>
                  ...
                  <View className="flex-row flex-wrap gap-2">
                    {SYMPTOM_OPTIONS.map((symptom) => (
                      <PressableScale
                        ...
                        style={{
                          backgroundColor: log.symptoms.includes(symptom)
                            ? "#7C6FCD"
                            : "#F5F0FF",
                      ...
                      >
                        <Text
                          className="text-sm font-medium"
                          style={{
                            color: log.symptoms.includes(symptom)
                              ? "#fff"
                              : "#9ca3af",
                          }}
                        >
          ...
          <View className="mt-5">
            {collapsed ? (
              ...
              <PressableScale
                ...
                style={{
                  backgroundColor:
                    saving || !hasSelection ? "#E5E7EB" : "#7C6FCD",
                }}
              >
                <Text
                  className="font-semibold text-base"
                  style={{
                    color: saving || !hasSelection ? "#9ca3af" : "#fff",
                  }}
                >
